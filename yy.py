...
##################################################
    ...
    def view_clients(self):
        if getattr(self, 'client_window', None) is not None and self.client_window.winfo_exists():
            self.client_window.lift()
            self.client_window.focus_force()
            return

        self.client_window = tb.Toplevel(self.root)
        self.client_window.title("Saved Clients")
        ...
        self.client_window.protocol("WM_DELETE_WINDOW", on_close)
        ...
        tree = tb.Treeview(self.client_window, columns=columns, show="headings", bootstyle="info")
        for col in columns:
        ...
        def show_receipts(treeview):
            ...
            name, email, _ = item['values']
            safe_name = name.replace(" ", "_") if name else email.split("@")[0]
            receipt_dir = os.path.join(FILE_DIR, "receipts")

            if not os.path.exists(receipt_dir):
                messagebox.showinfo("No Receipts", "No receipts found.")
                return

            matched_files = [
                f for f in os.listdir(receipt_dir)
                if f.lower().startswith(safe_name.lower()) and f.lower().endswith(".pdf")
            ]

            if not matched_files:
                messagebox.showinfo("No Receipts", f"No receipts found for {safe_name}.")
                return

            popup = tb.Toplevel(self.client_window)
            popup.title(f"Receipts for {name or email}")
            popup.geometry("500x400")

            receipt_listbox = tk.Listbox(popup, height=15)
            receipt_listbox.pack(fill="both", expand=True, padx=10, pady=10)

            for filename in matched_files:
                receipt_listbox.insert("end", filename)

            btn_frame = tb.Frame(popup)
            btn_frame.pack(pady=5)

            def open_selected():
                idx = receipt_listbox.curselection()
                if not idx:
                    return
                receipt_path = os.path.join(receipt_dir, receipt_listbox.get(idx[0]))
            ...
            def email_selected():
                ...
                if not idx:
                    return
                filepath = os.path.join(receipt_dir, receipt_listbox.get(idx[0]))
                subject = "Your Receipt"
                message = f"Hello {name},\n\nAttached is your receipt."
                self.send_selected_product(
                    to_email=email,
                    subject=subject,
            ...
            def archive_selected():
                ...
                if not idx:
                    return
                filename = receipt_listbox.get(idx[0])
                src_path = os.path.join(receipt_dir, filename)
                archive_dir = os.path.join(receipt_dir, "archived")
                os.makedirs(archive_dir, exist_ok=True)
                shutil.move(src_path, os.path.join(archive_dir, filename))
                receipt_listbox.delete(idx[0])
                messagebox.showinfo("Archived", f"{filename} archived.")

            tb.Button(btn_frame, text="Open", bootstyle=PRIMARY, command=open_selected).pack(side="left", padx=5)
            tb.Button(btn_frame, text="Email", bootstyle=INFO, command=email_selected).pack(side="left", padx=5)
            tb.Button(btn_frame, text="Archive", bootstyle=SECONDARY, command=archive_selected).pack(side="left",
                                                                                                     padx=5)

        # ---- Right-click menu ----
        menu = tb.Menu(self.client_window, tearoff=0)
        ...
        menu.add_separator()
        menu.add_command(label="View Receipts", command=lambda: show_receipts(tree))
        ...
        # ---- View Receipts Function ----
        def view_receipts(treeview):
            selected = treeview.selection()
            if not selected:
            ...
            client_name = treeview.item(client_id)["values"][0] or "No_Name"
            receipt_dir = os.path.join(FILE_DIR, "receipts")

            # Match PDFs that contain client name
            matches = [
                os.path.join(receipt_dir, f) for f in os.listdir(receipt_dir)
                if f.lower().endswith(".pdf") and client_name.replace(" ", "_") in f
            ]

            if not matches:
                messagebox.showinfo("No Receipts", f"No receipts found for {client_name}")
                return
            ...
            if os.name == 'nt':  # Windows
                ...
                subprocess.Popen(['xdg-open', receipt_dir])
            else:
                webbrowser.open(receipt_dir)
#########################################################################
...
#######################################################################
    def send_selected_product(self, to_email=None, subject=None, message_body=None):
        import smtplib
        from email.message import EmailMessage
        ...
        def send_action():
            ...
            try:
                ...
                # 📦 Generate receipt if price is entered
                price_str = price_var.get().strip()
                discount = float(discount_var.get().strip() or 0)
                tax_percent = float(tax_var.get().strip() or 0)
                client_name = to.split("@")[0]

                if price_str:
                    try:
                        items = [(file_name, float(price_str))]

                        # Ask user to select logo (optional)
                        logo_path = filedialog.askopenfilename(
                            title="Select Logo Image (Optional)",
                            filetypes=[("Image files", "*.png *.jpg *.jpeg *.bmp *.gif"), ("All files", "*.*")]
                        )
